...
_controller2.default.init(_socket2.default);

});

require.register("js/controller.js", function(exports, require, module) {
"use strict";
...
var controller = {
	...
	init: function init(socket) {
		...
		var score_indicator = document.getElementById("score-indicator");
		var lives_indicator = document.getElementById("lives-indicator");
		...
		this.channel.on("score", function (resp) {
			score_indicator.innerHTML = " " + resp.payload;
		});

		this.channel.on("position", function (resp) {
			var slider_position = Math.round(resp.payload / 10);
		...
		this.channel.on("lives", function (resp) {
			var number_of_lives = resp.payload;
			var new_game_indicator = document.getElementById("new-game-indicator");
		...
		var slider = document.getElementById("slider-indicator");
		var left_extreme = lives_indicator.getBoundingClientRect();
		left_extreme = left_extreme.left;
		var right_extreme = score_indicator.getBoundingClientRect();
		...
		document.onmousemove = function (event) {
			...
			var payload = { payload: mouse_position };

			_this.channel.push("set_position", payload);
		};

		document.onclick = function (event) {
			var number_of_lives = parseInt(lives_indicator.innerHTML.trim(), 10);

			if (number_of_lives != 0) {
				_this.channel.push("fire", { payload: null });
			} else {
				_this.channel.push("new_game", { payload: null });
			}
		};
	}
};
